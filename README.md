...
> 当前 GitHub 仓库开源的是 **轻量版**。`猫之琴终极版` 是独立版本，不在本仓库中开源，这里仅作介绍。
...
| --- | --- | --- | --- | --- |
| 1 | BS-Roformer-Leap | 音频分轨 | 人声 / 伴奏（2 音轨） | 分离纯净度顶尖，人声与伴奏互不串扰；支持多次迭代处理和提纯，适合对人声或伴奏纯净度要求极高的场景。 |
| 2 | DemucsV4 6s | 音频分轨 | 人声 / 鼓 / 贝斯 / 吉他 / 钢琴 / 其他（6 音轨） | 市面主流分轨方案，拆分维度更细；适合提取特定乐器轨，例如单独扒钢琴或贝斯后进行转录。 |
| 3 | BasicPitch | 通用转录 | 音频转 MIDI | 入门级转录模型，很多收费网站都在使用；仅靠 CPU 即可推理，体积小、速度快、资源占用低，适合快速试听、批量处理或低配置电脑。 |
| 4 | GAME 系列 | 人声转录 | 短句 / 人声 / 哼唱 / 清唱 / 主旋律转 MIDI | 适合几十秒的短素材，不建议转换大文件或长时间歌曲。<br>GAME-S：速度最快，显存占用最低，生成质量不错。<br>GAME-M：速度中等，显存占用中等，生成质量较好。<br>GAME-L：速度最慢，显存占用最高，生成质量最好。 |
| 5 | Transkun | 钢琴高精度转录 | 纯钢琴曲精准转 MIDI | 采用 Transformer + Neural Semi-CRF 架构，MIREX 比赛级水准，F1 分数 0.9081；默认不通过踏板延长音符，时值更贴近真实演奏动作。 |
| 6 | PianoTrans | 钢琴高精度转录 | 纯钢琴曲精准转 MIDI | 字节跳动 CNN + RNN 高分辨率模型，具备高精度钢琴转录能力，并支持延音踏板事件识别输出，适合保留完整踏板细节。 |
| 7 | Pop2Piano | AI 钢琴改编 | 流行歌曲生成钢琴独奏 MIDI | 不识别原有钢琴，而是将完整流行歌曲通过 AI 创作成全新的钢琴独奏曲，适合快速试听钢琴改编版灵感。 |
